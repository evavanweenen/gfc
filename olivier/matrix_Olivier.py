...
def add_w(t, plx_col = "parallax", mura_col = "pmra", mudec_col = "pmdec", v_r_col = None, components = True, vector = True):
    ...
    if components:
        ...
        t.add_column(table.Column(data = ws[:, 1], name = "w2", unit = "km / s"))
        t.add_column(table.Column(data = ws[:, 2], name = "w3", unit = "km / s"))
    if vector:
...
def transformation(t, v_rad_col = None):
    """
    Input units: ra (mas), dec (mas), prlx (mas), pm (mas/yr), pm (mas/yr)
    """
    #gfc.remove_unused_columns(t_astr)
    add_w(t, v_r_col = v_rad_col) #w-array (km/s, km/s, km/s)
    add_A(t)
    add_R(t)
    add_C(t)
    add_Q(t)
